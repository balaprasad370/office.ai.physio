    ...
    <div className="flex flex-col gap-2 border border-n-12 rounded-lg p-2 h-full">
      <div className="flex justify-between items-center">
        ...
        <button
          ...
          className="px-2 py-1 text-xs font-medium text-n-1 bg-color-1 rounded-md hover:bg-opacity-50"
        >
          Configure
        </button>
      </div>
      ...
      {voiceStats.status === "Not Active" ? (
        <div className="flex flex-col items-center justify-center py-6 px-4 rounded-lg border border-n-12 bg-n-7">
          ...
          <h3 className="text-base font-medium text-n-1 mb-1">
            Voice Agent Not Active
          </h3>
          <button
            onClick={() => router.push("/dashboard/voice")}
            className="px-3 py-1.5 text-sm text-n-1 bg-color-1 rounded-md"
          >
            Activate
          </button>
        </div>
        ...
        <>
          <div className="grid grid-cols-2 gap-2">
            <div className="p-2 rounded-lg border border-n-12 bg-n-7">
              <div className="flex items-center gap-1 mb-1">
                <div
                  className={`w-2 h-2 rounded-full ${voiceStats.status === "Active" ? "bg-color-4" : "bg-color-2"}`}
                />
                <span className="text-xs text-n-1">
                  Status: {voiceStats.status}
                </span>
              </div>
          ...
          <div className="p-2 rounded-lg border border-n-12 bg-n-7">
            <div className="flex justify-between items-center mb-1">
              ...
              <button
                ...
                className="text-xs text-color-1 hover:underline hover:underline-offset-4 hover:text-color-1/90"
              >
                Manage
              </button>
            </div>
            {voiceStats.activeNumbers.map((number, index) => (
              <div
                key={index}
                className="flex items-center gap-1 text-xs text-n-2"
              >
          ...
          <div className="flex flex-col gap-1">
            <div className="flex justify-between items-center">
              ...
              <button
                onClick={() => router.push("/dashboard/voice/calls")}
                className="text-xs text-color-1 hover:underline hover:underline-offset-4 hover:text-color-1/90"
              >
                View All
              </button>
            ...
            {recentCalls.map(call => (
              <div
                key={call.id}
                className="p-2 rounded-lg border border-n-12 bg-n-7"
              >
                <div className="flex justify-between items-center mb-1">
