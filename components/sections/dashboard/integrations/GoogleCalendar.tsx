...
const Integrations = () => {
  const [googleCalendarDetails, setGoogleCalendarDetails] = useState(null)
  const [googleCalendarStatus, setGoogleCalendarStatus] = useState(false)

  const handleGoogleCalendarIntegration = async () => {
    try {
      const response = await apiClient.get(`/client/tokenDetails`)
      window.location.href = "auth/google/" + response.data.clientId
    } catch (error) {
      console.log(error)
  ...
  const getGoogleIntegrationStatus = async () => {
    try {
      const response = await apiClient.get(`/integrations/googlecalendar`)

      if (response.data.status) {
        setGoogleCalendarDetails(response.data.data)
  ...
  const handleGoogleCalendarIntegrationAction = async action => {
    try {
      const response = await apiClient.put(
        `/integrations/googlecalendar/action/${action}`,
      )
    ...
    <div className="container mx-auto px-4">
      <div className="flex min-h-[200px] flex-col p-4 ">
        <div className="flex flex-row items-center justify-between gap-4">
          ...
          <div>
            {googleCalendarStatus ? (
              <button
                onClick={() => handleGoogleCalendarIntegrationAction("pause")}
              ...
              <button
                onClick={() => handleGoogleCalendarIntegrationAction("resume")}
                className="px-4 py-2 border rounded-md hover:bg-gray-100"
              >
                Resume
              </button>
        ...
        <button
          ...
          className="mt-4 flex w-full items-center justify-center gap-2 border rounded-md p-2 hover:bg-gray-100"
        >
          <svg
            width="15"
            height="15"
          ...
          {googleCalendarStatus ? "Connected" : "Connect"}
        </button>
      </div>
    </div>
  )
