...
const Integrations = () => {
  ...
  const handleGoogleCalendarIntegration = async () => {
    try {
      const response = await axios.get(`${apiEndPoint}client/tokenDetails`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      })
      window.location.href =
        apiEndPoint + 'auth/google/' + response.data.clientId
    } catch (error) {
      console.log(error)
  ...
  const getGoogleIntegrationStatus = async () => {
    try {
      const response = await axios.get(
        `${apiEndPoint}integrations/googlecalendar`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const handleGoogleCalendarIntegrationAction = async (action) => {
    try {
      const response = await axios({
        method: 'put',
        url: `${apiEndPoint}integrations/googlecalendar/action/${action}`,
        headers: {
          Authorization: `Bearer ${token}`,
        },
      })
